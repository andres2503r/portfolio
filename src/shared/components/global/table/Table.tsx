...
export default function Table<T> ({ enableTableFooter, tableHeight, enableTableFilter, availableFilters, TableAction, enableTablePagination, onRowSelection, scrollAction, NotDataComponent, clearState, actionClear, ...props }: ITable<T>) {
  ...
  return (
    ...
    >
      ...
      <div className='w-[86vw] h-[90vh] 2xl:w-[84vw] sm:w-[83vw] rounded-lg  overflow-y-auto' style={customStyles.containerTable}>
        <table className="table flex-col  ">
          <thead className="sticky z-20 bg-primary  shadow-xl  top-0 w-full">
              ...
              <tr key={headerGroup.id}>
                ...
                {headerGroup.headers.map(header => (
                  <th
                    ...
                    className={`ps-5 h-[40px] 2xl:h-[73px] relative text-[15px]  first:rounded-tl-[8px]  2xl:text-[24px] bg-primary text-background ${
                  ...
                  >
                    <div
                      className={`flex items-center bg-primary rounded-lg overflow-hidden text-ellipsis whitespace-nowrap${
                        (header.column.id === 'state' || header.column.id === 'options' || header.column.id === 'file.fileState')
                    ...
                    >
                      ...
                      <div
                        ...
                        className={`absolute right-0 top-0 h-full rounded-lg  w-[2px] bg-primary cursor-col-resize select-none touch-none transition-opacity ${
                          header.column.getIsResizing()
                            ? 'bg-primary rounded-lg'
                            : 'border-r-2 border-primary rounded-lg  hover:border-primary'
                        }`}
